class RecruitingEventsController < ApplicationController
  def index
    ...
    @recruiting_events = @q.result(:distinct => true).includes(:firm, :event_rsvps, :interactions, :setting).page(params[:page]).per(10)
  ...
  def create
    ...
    @recruiting_event.time = params[:time]
    @recruiting_event.is_closed = params[:is_closed]
